...
from dns.dnscurve_operations import dnscurve_encode_streamlined
from lib.crypto import nacl
from lib.packet.path import CorePath
from lib.packet.scion import SCIONPacket, get_type
from lib.packet.host_addr import IPv4HostAddr, SCIONHostAddr
...
QTYPE = Bimap('QTYPE',
                {1:'A', 2:'NS', 5:'CNAME', 6:'SOA', 12:'PTR', 15:'MX',
                 ...
                 51:'NSEC3PARAM', 52:'TLSA', 55:'HIP', 99:'SPF',100:'SCIONA', 249:'TKEY',
...
class DNSRecord(object):
    ...
    def __init__(self, header=None, questions=None,
        ...
        self.set_header_qa()
        self.is_curve = None
        self.dnscurve_nonce = None
        self.dnscurve_pk = None
        self.dnscurve_sk = None
        self.dnscurve_second_pk = None
    ...
    def send(self, src, dst, port=30040, tcp=False, timeout=None, sd = None):
        """
            Send packet to name server and return response
        """
        data = self.pack()
        data = bytes(data)
        if self.is_curve:
            cyphertext_box = nacl.crypto_box_curve25519xsalsa20poly1305(data, self.dnscurve_nonce, self.dnscurve_second_pk, self.dnscurve_sk)
            packet = dnscurve_encode_streamlined(self.dnscurve_nonce, cyphertext_box, self.dnscurve_pk)
            data = packet
        if sd is None:
            address_source_ipv4 = IPv4HostAddr(src)
            path = None
        else: 
            _,isd, ad, _ = dst.split(".")
            address_source_ipv4 = IPv4HostAddr(src)
            paths = sd.get_paths(int(isd),int(ad))
            path = paths[0]
        address_destination_ipv4 = IPv4HostAddr(dst)
        scion_request_packet = SCIONPacket.from_values(src=address_source_ipv4, dst=address_destination_ipv4, payload=data, path=path, pkt_type=PT.DNS_REQ)
        ...
        if sd is None:
            next_hop = dst
            sock.sendto(scion_request_packet.pack(), (str(next_hop), port))
            packet , _ = sock.recvfrom(8192)

        else:
            (next_hop, port) =sd.get_first_hop(scion_request_packet)
            sd.send(scion_request_packet, next_hop, port)
            receive_sock.bind(((str(address_source_ipv4)), SCION_UDP_EH_DATA_PORT))
            packet, server = receive_sock.recvfrom(8192)
        ...
        response = spkt.payload        
        #TODO: Implement a loop that does not answer before having
        #A valid DNScurve answer reply from the nameserver.
        if response[:8] == b'R6fnvWj8':
            is_curve = True
            dnscurve_answer_nonce = response[8:32]
            if dnscurve_answer_nonce[:12] != self.dnscurve_nonce[:12]:
                raise ValueError("Nonces are not matching.")
            dnscurve_answer_box = response[32:]
            data = nacl.crypto_box_curve25519xsalsa20poly1305_open(dnscurve_answer_box, dnscurve_answer_nonce, self.dnscurve_second_pk, self.dnscurve_sk)
            response = data
        return response
...
class SCIONA(RD):
    """
        Basic support for SCION  A record.
        We use the SCIONHostAddr type to represent SCION addresses.
    """
    
    def __init__(self, data):
        self.data = data
    
    #FIXME: Because the SCION host addresses are not used yet,
    # I decide to parse it like simple text information.
    @classmethod
    def parse(cls, buffer, length):
        try:
            (scionalength,) = buffer.unpack("!B")

            if scionalength < length:
                data = buffer.get(scionalength)
            else:
                raise DNSError("Invalid SCION A record: len(%d) > RD len(%d)" % 
                                        (scionalength, length))
            return cls(data)
        except (BufferError, BimapError) as e:
            raise DNSError("Error unpacking SCION A [offset=%d]: %s" % 
                                        (buffer.offset, e))
    @classmethod
    def fromZone(cls, rd, origin=None):
        return cls(rd[0].encode())
    ...
    def pack(self, buffer):
        if len(self.data) > 255:
            raise DNSError("SCION A record too long: %s" % self.data)
        buffer.pack("!B", len(self.data))
        if(isinstance(self.data, str)):
            buffer.append(self.data.encode("utf-8"))
        else:
            buffer.append(self.data)
...
RDMAP = { 'CNAME':CNAME, 'A':A,'SCIONA':SCIONA,'AAAA':AAAA, 'TXT':TXT, 'MX':MX,
